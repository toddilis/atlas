import type { Decision, Payment, PolicyState, Rule, RuleConfig, RuleResult } from './types.js';
...
/**
 * Evaluate a payment against the rule set.
 *
 * Precedence: `block` > `escalate` > `allow`. The `reasons` array carries the explanation
 * from every rule that contributed to the final decision (i.e. every blocking or
 * escalating rule; `allow` results are omitted unless the final decision is `allow` and
 * the array would otherwise be empty, in which case a single "all rules cleared" entry
 * is included for the audit log).
 */
export function evaluate(payment: Payment, state: PolicyState, config: RuleConfig): Decision {
  const results: RuleResult[] = [];
  for (const rule of RULES) {
    const r = rule(payment, state, config);
    if (r) results.push(r);
  }
  ...
  const ranAtLeastOne = results.length > 0;
  return {
    decision: 'allow',
    reasons: ranAtLeastOne
